    ...
    public partial class BtShared
    {
        internal RC allocateBtreePage(ref MemPage ppPage, ref Pgno pPgno, Pgno nearby, byte exact)
        {
            ...
            if (n > 0)
            {
                ...
                do
                {
                    ...
                    if (k == 0 && searchList == 0)
                    {
                        ...
                        rc = Pager.sqlite3PagerWrite(pTrunk.DbPage);
                        if (rc != RC.OK)
                            goto end_allocate_page;
                        pPgno = iTrunk;
                    ...
                    else if (searchList != 0 && nearby == iTrunk)
                    {
                        ...
                        if (k == 0)
                        {
                            ...
                            {
                                rc = Pager.sqlite3PagerWrite(pPrevTrunk.DbPage);
                                if (rc != RC.OK)
                                    goto end_allocate_page;
                        ...
                        else
                        {
                            ...
                            if (rc != RC.OK)
                                goto end_allocate_page;
                            rc = Pager.sqlite3PagerWrite(pNewTrunk.DbPage);
                            if (rc != RC.OK)
                            {
                            ...
                            if (pPrevTrunk == null)
                            {
                                Debug.Assert(Pager.sqlite3PagerIswriteable(pPage1.DbPage));
                    ...
                    else if (k > 0)
                    {
                        ...
                        if (searchList == 0 || iPage == nearby)
                        {
                            ...
                            if (rc == RC.OK)
                            {
                                rc = Pager.sqlite3PagerWrite((ppPage).DbPage);
                                if (rc != RC.OK)
                                    ppPage.releasePage();
                            }
                            searchList = 0;
            ...
            else
            {
                ...
                rc = Pager.sqlite3PagerWrite(this.Page1.DbPage);
                if (rc != RC.OK)
                    return rc;
                this.Pages++;
        ...
        end_allocate_page:
            ...
            if (rc == RC.OK)
            {
                if (Pager.sqlite3PagerPageRefcount((ppPage).DbPage) > 1)
                {
                    ppPage.releasePage();
                    return SysEx.SQLITE_CORRUPT_BKPT();
                }
                ...
                ppPage = null;
            Debug.Assert(rc != RC.OK || Pager.sqlite3PagerIswriteable((ppPage).DbPage));
            return rc;
        }
        ...
        internal RC freePage2(MemPage pMemPage, Pgno iPage)
        {
            ...
            {
                pPage = pMemPage;
                Pager.sqlite3PagerRef(pPage.DbPage);
            }
            else
                pPage = btreePageLookup(iPage);
            // Increment the free page count on pPage1
            var rc = Pager.sqlite3PagerWrite(pPage1.DbPage);
            if (rc != RC.OK)
                goto freepage_out;
            ...
            if (this.SecureDelete)
            {
                ...
                if ((pPage == null && ((rc = btreeGetPage(iPage, ref pPage, 0)) != RC.OK)) || ((rc = Pager.sqlite3PagerWrite(pPage.DbPage)) != RC.OK))
                    goto freepage_out;
...
if (false)
                ...
                if (nLeaf < (uint)this.UsableSize / 4 - 8)
                {
                    ...
                    if (rc == RC.OK)
                    {
                        ...
                        if (pPage != null && !this.SecureDelete)
                            Pager.sqlite3PagerDontWrite(pPage.DbPage);
            ...
            if (pPage == null && (rc = btreeGetPage(iPage, ref pPage, 0)) != RC.OK)
                goto freepage_out;
            rc = Pager.sqlite3PagerWrite(pPage.DbPage);
            if (rc != RC.OK)
                goto freepage_out;
