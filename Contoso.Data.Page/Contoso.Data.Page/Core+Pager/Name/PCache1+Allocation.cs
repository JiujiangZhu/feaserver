...
namespace Contoso.Core.Name
{
    ...
    {
        internal static void sqlite3PCacheBufferSetup(object pBuf, int sz, int n)
        {
            if (pcache1.isInit)
            {
        ...
        internal static PgHdr pcache1Alloc(int nByte)
        {
            PgHdr p = null;
        ...
        internal static void pcache1Free(ref PgHdr p)
        {
            if (p == null)
                return;
            if (p.CacheAllocated)
            ...
            else
            {
                ...
                var iSize = MallocEx.sqlite3MallocSize(p.pData);
                MutexEx.sqlite3_mutex_enter(pcache1.mutex);
                StatusEx.sqlite3StatusAdd(StatusEx.STATUS.PAGECACHE_OVERFLOW, -iSize);
                MutexEx.sqlite3_mutex_leave(pcache1.mutex);
                MallocEx.sqlite3_free(ref p.pData);
            }
        }

#if SQLITE_ENABLE_MEMORY_MANAGEMENT
        internal static int pcache1MemSize(object p)
        {
        ...
        internal PgHdr1 pcache1AllocPage()
        {
        ...
        internal static void pcache1FreePage(ref PgHdr1 p)
        {
            if (Check.ALWAYS(p))
            {
                var pCache = p.pCache;
        ...
        internal static PgHdr sqlite3PageMalloc(int sz) { return pcache1Alloc(sz); }

        internal static void sqlite3PageFree(ref byte[] p)
        {
            if (p != null)
            {
        ...
        internal static void sqlite3PageFree(ref PgHdr p) { pcache1Free(ref p); }

        internal bool pcache1UnderMemoryPressure() { return (pcache1.nSlot != 0 && szPage <= pcache1.szSlot ? pcache1.bUnderPressure : MallocEx.sqlite3HeapNearlyFull()); }
    }
}
