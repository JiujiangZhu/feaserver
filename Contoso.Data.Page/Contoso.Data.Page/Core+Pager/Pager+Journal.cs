...
//  int sqlite3JournalCreate(sqlite3_file );
        ...
        internal static RC readMasterJournal(VirtualFile pJrnl, byte[] zMaster, uint nMaster)
        {
        ...
        internal RC zeroJournalHdr(int doTruncate)
        {
            var rc = RC.OK;
        ...
        internal RC readJournalHdr(int isHot, long journalSize, out uint pNRec, out Pgno pDbSize)
        {
            var aMagic = new byte[8]; // A buffer to hold the magic header
            Debug.Assert(this.jfd.isOpen);    // Journal file must be open.
            pNRec = 0;
        ...
        internal RC writeMasterJournal(string zMaster)
        {
        ...
        internal RC pagerSyncHotJournal()
        {
            var rc = RC.OK;
            if (!this.noSync)
        ...
        internal RC syncJournal(int newHdr)
        {
            var rc = RC.OK;
        ...
        internal static RC subjournalPage(PgHdr pPg)
        {
            var rc = RC.OK;
            var pPager = pPg.pPager;
        ...
        internal RC hasHotJournal(ref int pExists)
        {
            var pVfs = this.pVfs;
