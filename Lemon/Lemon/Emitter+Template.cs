        ...
        public class Template
        {
            ...
            * if name!=0, then any word that begin with "Parse" is changed to begin with *name instead. */
            ...
            internal static StreamReader tplt_open(Context ctx)
            {
                ...
                if (!string.IsNullOrEmpty(user_templatename))
                {
                    if (File.Exists(user_templatename))
                    ...
                    try { r = fileInfo.Open(FileMode.Open, FileAccess.Read); }
                    catch
                    {
                ...
                var templateName = Path.Combine(Path.GetDirectoryName(filePath), Path.GetFileNameWithoutExtension(filePath) + ".lt");
                if (File.Exists(templateName))
                    try { fileInfo = new FileInfo(templateName); }
                    catch { fileInfo = null; }
                if (fileInfo == null)
                {
                    templateName = Path.Combine(Path.GetDirectoryName(filePath), _templateName);
                    if (File.Exists(templateName))
                        try { fileInfo = new FileInfo(templateName); }
                        catch { fileInfo = null; }
                }
                ...
                {
                    Console.WriteLine("Can't open the template file \"{0}\".", templateName);
                    ctx.Errors++;
                    return null;
                }
                return new StreamReader(r);
            ...
            internal static void tplt_print(StreamWriter w, Context ctx, string value, ref int lineno)
            {
                ...
                if (!ctx.NoShowLinenos) { lineno++; tplt_linedir(w, lineno, ctx.Outname ?? ctx.Filename); }
            }
        }
    }
}