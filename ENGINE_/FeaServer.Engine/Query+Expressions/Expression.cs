...
namespace FeaServer.Engine.Query
{
    public class Expressions
    {
        ...
        // pExpr is an operand of a comparison operator.  aff2 is the type affinity of the other operand.  This routine returns the
        // type affinity that should be used for the comparison operator.
        internal static char sqlite3CompareAffinity(Expr pExpr, char aff2)
        {
            var aff1 = sqlite3ExprAffinity(pExpr);
            if (aff1 != '\0' && aff2 != '\0')
            {
                // Both sides of the comparison are columns. If one has numeric affinity, use that. Otherwise use no affinity.
                // if (sqlite3IsNumericAffinity(aff1) || sqlite3IsNumericAffinity(aff2))        
                return (aff1 >= SQLITE_AFF_NUMERIC || aff2 >= SQLITE_AFF_NUMERIC ? SQLITE_AFF_NUMERIC : SQLITE_AFF_NONE);
            }
            else if (aff1 == '\0' && aff2 == '\0')
            {
                // Neither side of the comparison is a column.  Compare the results directly.
        ...
        // pExpr is a comparison operator.  Return the type affinity that should be applied to both operands prior to doing the comparison.
        internal static byte comparisonAffinity(Expr pExpr)
        {
            Debug.Assert(pExpr.op == TK.EQ || pExpr.op == TK.IN || pExpr.op == TK.LT ||
                pExpr.op == TK.GT || pExpr.op == TK.GE || pExpr.op == TK.LE ||
                pExpr.op == TK.NE || pExpr.op == TK.IS || pExpr.op == TK.ISNOT);
            Debug.Assert(pExpr.pLeft != null);
            var aff = sqlite3ExprAffinity(pExpr.pLeft);
            if (pExpr.pRight != null)
                aff = sqlite3CompareAffinity(pExpr.pRight, aff);
            else if (Expr.ExprHasProperty(pExpr, Expr.EP_xIsSelect))
        ...
        // pExpr is a comparison expression, eg. '=', '<', IN(...) etc. idx_affinity is the affinity of an indexed column. Return true
        // if the index with affinity idx_affinity may be used to implement the comparison in pExpr.
        internal static bool sqlite3IndexAffinityOk(Expr pExpr, char idx_affinity)
        {
            var aff = comparisonAffinity(pExpr);
            switch (aff)
            {
                case SQLITE_AFF_NONE: return true;
                case SQLITE_AFF_TEXT: return idx_affinity == SQLITE_AFF_TEXT;
                default: return idx_affinity >= SQLITE_AFF_NUMERIC;// sqlite3IsNumericAffinity(idx_affinity);
            }
        }

        // Return the P5 value that should be used for a binary comparison opcode (OP_Eq, OP_Ge etc.) used to compare pExpr1 and pExpr2.
        internal static byte binaryCompareP5(Expr pExpr1, Expr pExpr2, int jumpIfNull)
        {
            var aff = (byte)sqlite3ExprAffinity(pExpr2);
